        ...
        try {
            ...
            switch (validated.action) {
                ...
                case 'batch_get_user_info': {
                    ...
                    const userList = result.user_info_list.map(user => `- ${user.nickname} (${user.openid}) - ${user.subscribe === 1 ? '已关注' : '未关注'}`).join('\n');
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'get_user_summary': {
                    ...
                    const summary = result.list.map(item => `${item.ref_date}: 新增 ${item.new_user} 人，取消 ${item.cancel_user} 人`).join('\n');
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'get_user_cumulate': {
                    ...
                    const cumulate = result.list.map(item => `${item.ref_date}: 累计用户 ${item.cumulate_user} 人`).join('\n');
                    return {
                        content: [{
                                type: 'text',
