...
async function handlePublishTool(context) {
        ...
        switch (action) {
            ...
            case 'list': {
                ...
                try {
                    ...
                    const publishList = result.item.map((item, index) => {
...
async function handlePublishMcpTool(args, apiClient) {
    const { action, mediaId, publishId, offset = 0, count = 20 } = args;
    try {
        switch (action) {
            case 'submit': {
                if (!mediaId) {
