...
const mediaUploadToolSchema = z.object({
    ...
    fileData: z.string().max(10485760, '文件数据过大').optional(),
    fileName: z.string().max(256, '文件名过长').optional(),
    mediaId: mediaIdSchema.optional(),
    title: z.string().max(64, '标题过长').optional(),
    introduction: z.string().max(512, '描述过长').optional(),
});
async function handleMediaUploadTool(args, apiClient) {
    const { action, type, filePath, fileData, fileName, mediaId, title, introduction } = args;
    try {
        switch (action) {
            case 'upload': {
                if (!type) {
                ...
                const result = await apiClient.post(`/cgi-bin/media/upload?type=${type}`, formData);
            ...
            case 'get': {
                ...
                try {
                    const result = await apiClient.get(`/cgi-bin/media/get?media_id=${mediaId}`);
                    return {
                        content: [{
                                type: 'text',
                                text: `获取临时素材成功！\n素材ID: ${mediaId}\n素材已下载到本地`,
