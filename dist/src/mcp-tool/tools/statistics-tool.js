...
export const statisticsMcpTool = {
        ...
        try {
            ...
            switch (validated.action) {
                case 'get_article_summary': {
                    ...
                    const summary = result.list.map(item => `${item.refDate}:\n` +
                ...
                case 'get_article_total': {
                    ...
                    const total = result.list.map(item => `${item.refDate}:\n` +
                        `  - 来源: ${item.userSource}\n` +
                        `  - 阅读人数: ${item.readUser}\n` +
                        `  - 阅读次数: ${item.readCount}\n` +
                        `  - 分享人数: ${item.shareUser}\n` +
                        `  - 分享次数: ${item.shareCount}`).join('\n\n');
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'get_user_read': {
                    ...
                    const read = result.list.map(item => `${item.refDate}:\n` +
                        `  - 图文页阅读人数: ${item.intPageReadUser}\n` +
                        `  - 图文页阅读次数: ${item.intPageReadCount}\n` +
                        `  - 原文页阅读人数: ${item.oriPageReadUser}\n` +
                        `  - 原文页阅读次数: ${item.oriPageReadCount}\n` +
                        `  - 分享/收藏人数: ${item.shareUser}/${item.addToFavUser}\n` +
                        `  - 分享/收藏次数: ${item.shareCount}/${item.addToFavCount}`).join('\n\n');
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'get_user_share': {
                    ...
                    const share = result.list.map(item => `${item.refDate}:\n` +
                        `  - 分享人数: ${item.shareUser}\n` +
                        `  - 分享次数: ${item.shareCount}`).join('\n');
                    return {
                        content: [{
                ...
                case 'get_upstream_message': {
                    ...
                    const message = result.list.map(item => `${item.refDate}:\n` +
                        `  - 消息类型: ${item.msgType}\n` +
                        `  - 上报发送用户数: ${item.msgUser}\n` +
                        `  - 上报发送消息数: ${item.msgCount}`).join('\n');
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'get_interface_summary_hour': {
                    ...
                    const summary = result.list.slice(0, 24).map(item => `${item.refDate} ${item.refHour}:00:\n` +
                        `  - 调用次数: ${item.callbackCount}\n` +
                        `  - 失败次数: ${item.failCount}\n` +
                        `  - 总耗时: ${item.totalTime}ms\n` +
                        `  - 最大耗时: ${item.maxTime}ms`).join('\n');
...
function parseStatisticsParams(params) {
    return params;
}
