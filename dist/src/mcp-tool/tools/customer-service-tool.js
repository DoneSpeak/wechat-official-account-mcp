...
export const customerServiceMcpTool = {
        ...
        try {
            ...
            switch (validated.action) {
                ...
                case 'get_records': {
                    ...
                    const result = await apiClient.getCustomMessageRecords(validated.startTime, validated.endTime, validated.msgId, validated.number);
                    const records = result.records.map((record, index) => `${index + 1}. ${record.worker} -> ${record.openid}\n` +
...
function parseCustomerServiceParams(params) {
    return params;
}
