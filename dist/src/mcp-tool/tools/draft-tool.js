...
async function handleDraftOperations(action, params, apiClient) {
    switch (action) {
        case 'add': {
        ...
        case 'list': {
            ...
            try {
                ...
                const draftList = result.item.map((item, index) => {
...
async function handleDraftTool(context) {
...
async function handleDraftMcpTool(args, apiClient) {
    const { action, mediaId, articles, offset = 0, count = 20 } = args;
    try {
        return await handleDraftOperations(action, { mediaId, articles, offset, count }, apiClient);
    }
    catch (error) {
